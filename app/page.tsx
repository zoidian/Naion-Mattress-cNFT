...
const statusOptions = ["EoL", "received", "stored", "processing", "landfill", "incinerated", "downcycled", "upcycled"]
...
export default function WasteCollectionPage() {
  ...
  const [activeView, setActiveView] = useState<"register" | "inventory">("register")
  ...
  const selectedSize = mattressSizes.find((s) => s.name === formData.mattressSize)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const mockNFTAddress = `${publicKey?.toString().slice(0, 8)}...${Math.random().toString(36).slice(2, 8)}`
    setMintedNFT(mockNFTAddress)
  ...
  const updateMattressStatus = (id: string, newStatus: string) => {
    const updatedInventory = inventory.map((item) => (item.id === id ? { ...item, status: newStatus } : item))
    setInventory(updatedInventory)
    localStorage.setItem("mattressInventory", JSON.stringify(updatedInventory))
  }
  ...
  const getConditionColor = (condition: string) => {
    if (condition.includes("Reusable")) return "bg-green-500/20 text-green-700 border-green-500/30"
    if (condition.includes("Non-reusable")) return "bg-orange-500/20 text-orange-700 border-orange-500/30"
    if (condition.includes("Contaminated")) return "bg-red-500/20 text-red-700 border-red-500/30"
      ...
      <header className="border-b border-border bg-card">
        <div className="container mx-auto flex items-center justify-between px-4 py-4">
          <div className="flex items-center gap-3">
            ...
            />
            <div>
              <h1 className="text-xl font-bold text-foreground">BIFFA Waste Collection</h1>
              <p className="text-sm text-muted-foreground">Blockchain Waste Transfer System</p>
            </div>
          </div>
      ...
      <main className="container mx-auto px-4 py-8">
        {activeView === "register" ? (
          <div className="mx-auto max-w-3xl">
            <Card className="border-2 border-border bg-card shadow-lg">
              <CardHeader className="border-b border-border bg-muted/30">
                <CardTitle className="text-2xl text-foreground">Waste Transfer Note (WTN)</CardTitle>
              ...
              <CardContent className="space-y-6 pt-6">
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="space-y-4 rounded-lg border border-border bg-muted/20 p-4">
                    <h3 className="font-semibold text-foreground">Collector Information</h3>
                  ...
                  {/* Mattress Details */}
                  <div className="space-y-4 rounded-lg border border-border bg-muted/20 p-4">
                    <h3 className="font-semibold text-foreground">Mattress Details</h3>
                    <div className="space-y-2">
                      <Label htmlFor="mattressSize">Mattress Size</Label>
                      <Select
                        value={formData.mattressSize}
                        onValueChange={(value) => setFormData({ ...formData, mattressSize: value })}
                        required
                      >
                        <SelectTrigger id="mattressSize" className="bg-background">
                    ...
                    {selectedSize && (
                      <div className="grid grid-cols-2 gap-4 rounded-md bg-accent/20 p-3">
                        <div>
                          ...
                          <p className="font-medium text-foreground">{selectedSize.dimensions} cm</p>
                        </div>
                        <div>
                          <p className="text-sm text-muted-foreground">Weight Range</p>
                          <p className="font-medium text-foreground">{selectedSize.weight} kg</p>
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-4 rounded-lg border border-border bg-muted/20 p-4">
                    <h3 className="font-semibold text-foreground">Material Composition</h3>
                    <RadioGroup
                      value={formData.materialComposition}
                      onValueChange={(value) => setFormData({ ...formData, materialComposition: value })}
                      required
                    >
                      {materialCompositions.map((material) => (
                        <div key={material} className="flex items-center space-x-2">
                          <RadioGroupItem value={material} id={material} />
                          <Label htmlFor={material} className="cursor-pointer font-normal">
                            {material}
                          </Label>
                        </div>
                      ))}
                  ...
                  <div className="space-y-4 rounded-lg border border-border bg-muted/20 p-4">
                    <h3 className="font-semibold text-foreground">Condition Assessment</h3>
                    <RadioGroup
                      value={formData.conditionAssessment}
                      onValueChange={(value) => setFormData({ ...formData, conditionAssessment: value })}
                      required
                    >
                  ...
                  >
                    {isSubmitting ? "Minting WTN NFT..." : "Mint Collected Waste"}
                  </Button>
                  ...
                  {mintedNFT && (
                    <div className="rounded-lg border-2 border-primary bg-primary/10 p-4">
                      <p className="mb-2 font-semibold text-foreground">✓ WTN Successfully Minted!</p>
                      <p className="text-sm text-muted-foreground">NFT Address: {mintedNFT}</p>
                    </div>
                  )}
                </form>
              </CardContent>
          ...
          <div className="mx-auto max-w-6xl">
            <Card className="border-2 border-border bg-card shadow-lg">
              <CardHeader className="border-b border-border bg-muted/30">
                <CardTitle className="text-2xl text-foreground">Mattress Inventory</CardTitle>
                <p className="text-sm text-muted-foreground">Track all registered mattresses in the recycling chain</p>
              </CardHeader>
              <CardContent className="pt-6">
                {inventory.length === 0 ? (
                  <div className="py-12 text-center">
                    <p className="text-muted-foreground">No mattresses registered yet</p>
                    <Button variant="outline" className="mt-4 bg-transparent" onClick={() => setActiveView("register")}>
                    ...
                    {inventory.map((item) => (
                      ...
                      >
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <Badge variant="outline" className={getConditionColor(item.conditionAssessment)}>
                          ...
                          <Badge className={`${getStatusColor(item.status)} text-white border-0`}>{item.status}</Badge>
                        </div>
